...
Agent replies begin beside the speaker's name, with subsequent lines aligned to the reply. Tool calls sit one level inward with a `↳` marker and muted text. Click a tool's title or output to toggle that result independently. Running tools show up to 12 lines of output by default; completed or failed tools fold all their details. Click to see the full result, then click again to hide it. Explicit expansion choices remain in effect. Finished tasks fold their working steps while keeping the final answer; click the answer or closing line to toggle those steps. Opening a task shows its conversation and tool titles with all tool results folded; click a tool to inspect its result. `Ctrl+O` expands or collapses all tasks and tool results, including those outside the viewport. Dragging across text copies it without toggling a block.
