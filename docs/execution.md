...
These permission modes govern operations routed through the host. Codex ACP uses its adapter's permissions and sandbox, and enabled Codex profiles are always available for execution. The former `nativeTools` execution switch is ignored in old settings and removed when settings are saved. Known native tasks take the exclusive workspace lock, including inspections. See the [live adapter findings](live-use-cases.md) and [follow-up controls](execution-controls.md).
...
| `contextFrom` | Saved task IDs whose complete replies are attached as source material. Other task results are not attached automatically. |
...
The result includes `taskId`, `runId`, `agent`, `status`, `summary`, `message`, `artifacts`, `blockers`, `verification`, `usage`, and `resultRef`. `message` is the complete worker reply. Instructions and `contextFrom` IDs are stored separately under the run's `results` directory; attached source replies stay in their original result records. Artifact and blocker lists are returned in full.

`end_turn` is a protocol termination signal, not proof of task completion. A report saying `blocked` or `partial` produces `blocked` or `incomplete`; cancellation and failure remain distinct. Answer tasks request no work-report boilerplate. A clean turn without a report is accepted as done, with verification marked `unreported`. `verification.source: agent_report` identifies the worker's claim, not an independent test performed by handsfree. Callers can add a verification task and inspect its evidence.
...
Unknown dependencies, cycles and duplicate IDs are rejected before work starts. Failed prerequisites block dependent work. Explicit dependency edges select the prerequisites’ complete replies as source context. Their task IDs are recorded in `contextFrom`; the original task and constraints are preserved without embedding prerequisite output in them. Identical requests with identical dependencies in one batch share one execution. A worker session never receives overlapping prompts. A workspace lock allows concurrent inspections but serializes changes against all work, including other ACP runtimes in the same host process. Independent handsfree processes require separate writable checkouts or external coordination.
...
`usage` returns total tokens, frontier tokens, known USD cost, estimated-call count, and calls without known prices. `/cost` also shows this accounting in the conversation. Conversation tool results include the complete worker replies. `task_result` can retrieve them again after a turn ends or a run restarts.
...
Conversation history, replies and selected source attachments are passed in full during normal execution. Result retrieval supports explicit paging. On context overflow, the conversation loop saves an exact checkpoint and retries with a smaller active window that preserves user instructions and latest worker evidence. Invalid JSON gets up to two repair requests, and a truncated response gets one replacement request; valid work steps remain unlimited. See [agent loop](agent-loop.md) for steering, optional plan mode and background agents. Model servers and adapters still have their own context and output capacities. Explicit user cancellation closes the affected connection immediately; a subsequent worker task opens a new connection. Approvals remain governed by the session's `ask`/`bypass` mode and can be withdrawn by cancellation. Command environments inherit the host environment and apply the caller's overrides.
