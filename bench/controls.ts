...
try {
  const codex = await runtime.executor.execute({ agent: 'codex', task: 'Reply exactly: Codex is available. Do not use tools or change files.', kind: 'answer' });
  check('Codex runs without a native-tool opt-in', codex.status === 'done' && runtime.pool.isOpen('codex'), codex);
