...
const Input = z.discriminatedUnion('operation', [
  ...
  z.object({ operation: z.literal('save'), key: z.string().min(1).max(80),
    kind: z.enum(['objective', 'constraint', 'decision', 'finding', 'open']), text: z.string().min(1).max(2000),
    sources: z.array(z.number().int().positive()).min(1).max(16), active: z.boolean().default(true) }),
]);
...
export class ContextTool implements Tool<z.infer<typeof Input>> {
  ...
  constructor(private readonly context: RunContext, private readonly maxChars: number) {}
  describe(): string {
  ...
  async run(input: z.infer<typeof Input>, ctx: ToolContext) {
    ...
    try {
      switch (input.operation) {
        case 'search': return { text: this.context.search(input.query, this.maxChars) || 'No matching context.' };
        case 'read': {
          const page = this.context.read(input.record, input.offset, this.maxChars - 80);
