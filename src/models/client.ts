...
/** Keep the SDK's caller cancellation while omitting its automatic deadline. */
class UnlimitedOpenAI extends OpenAI {
  override async fetchWithTimeout(url: RequestInfo, init: RequestInit | undefined, _ms: number, controller: AbortController): Promise<Response> {
...
export class LocalModel implements ChatClient {
  ...
  constructor(private readonly config: Config['orchestration']['local']) {
    this.client = new UnlimitedOpenAI({
