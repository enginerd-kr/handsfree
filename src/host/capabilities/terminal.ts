 ...
 * a command handsfree started and approved, whichever agent asked. The alternative is not "no commands" —
...
function append(terminal: Terminal, chunk: Buffer): void {
  ...
  while (terminal.bytes > terminal.limit) {
    ...
    let dropped: Buffer;
    if (terminal.chunks[0]!.byteLength <= terminal.bytes - terminal.limit) {
...
export function buildEnv(requested: { name: string; value: string }[]): NodeJS.ProcessEnv {
  return { ...process.env, ...Object.fromEntries(requested.map(({ name, value }) => [name, value])) };
}
