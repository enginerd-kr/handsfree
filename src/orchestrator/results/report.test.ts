...
- and a network mock`;
  ...
  it('preserves the full message when there is no report block', () => {
    ...
    expect(report.summary).toBe(long.replace(/\s+/g, ' ').trim());
  ...
  it('preserves the entire summary and every list item', () => {
    const text = `REPORT\nsummary: ${'x'.repeat(500)}\nopen: - a\n- b\n- c\n- d`;
    const report = parseReport(text);
    expect(report.summary.length).toBe(500);
    expect(report.open).toEqual(['a', 'b', 'c', 'd']);
  });
