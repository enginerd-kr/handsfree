...
import type { TokenCharge, TokenUsage } from '../../contracts/usage.js';
export type { TokenCharge, TokenUsage } from '../../contracts/usage.js';
export interface UsageLease {
  setModel(model: string): void;
  finish(charge: TokenCharge, failed?: boolean): TokenUsage;
}
...
export class UsageTracker {
  ...
  begin(source: string, model: string, frontier: boolean): UsageLease {
    let finished: TokenUsage | undefined;
    return {
      setModel: (selected) => { model = selected; },
      finish: (charge, failed = false) => finished ??= this.record(source, model, frontier, charge, failed),
    };
  }
