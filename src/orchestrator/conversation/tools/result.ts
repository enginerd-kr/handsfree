...
export class ResultTool implements Tool<{ taskId: number; offset: number }> {
  ...
  describe(): string { return 'task_result — read a completed task\'s recorded reply and details to explain, compare or inspect previous results without contacting the agent again. Input: {"taskId":1,"offset":0}. Follow nextOffset for another page. Write your own summary in a later agent prompt, or use agent.context_from to forward exact replies directly.'; }
  async run({ taskId, offset }: { taskId: number; offset: number }) {
    try {
      const page = this.executor.readResult(taskId, offset);
