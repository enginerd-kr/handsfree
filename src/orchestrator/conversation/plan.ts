...
Choose recipients, order and dependencies from the user's meaning and conversation. An independent group call collects separate answers concurrently where workspace scheduling permits. Batch already-decided independent work in one agent array or one response with several calls instead of spending a planning turn to announce each next agent. Wait for returned evidence before deciding dependent work. Work that depends on a previous answer needs a later call with that evidence: use agent.context_from for exact saved replies, or read task_result and put your own summary in the next prompt.
