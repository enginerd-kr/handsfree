...
export class TaskScheduler {
  ...
  private drain(): void {
    for (let at = 0; at < this.queue.length;) {
      ...
      if ([...this.active.values()].some(Boolean)) return;
...
export function workspaceScheduler(dir: string): { scheduler: TaskScheduler; release(): void } {
  let shared = workspaces.get(dir);
  if (!shared) { shared = { scheduler: new TaskScheduler(), owners: 0 }; workspaces.set(dir, shared); }
  shared.owners++;
  let released = false;
