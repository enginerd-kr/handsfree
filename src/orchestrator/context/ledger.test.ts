...
describe('renderHandoff with reports', () => {
  ...
  it('includes every task in the handoff', () => {
    const transcript = new Transcript();
    for (let n = 1; n <= 6; n++) {
    ...
    expect(handoff).toContain('Did thing 1.');
    expect(handoff).not.toContain('earlier tasks not listed');
    expect(handoff.length).toBeGreaterThan(600);
  });

  it('keeps the complete account of failed tasks', () => {
    const transcript = new Transcript();
    ...
    expect(handoff).toContain('Refused halfway.');
  });
});
