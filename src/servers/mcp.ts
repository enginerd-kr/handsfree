...
export function createMcpServer(runtime: Runtime): McpServer {
  ...
  server.registerTool('read_result', {
    description: 'Read the complete task result, or request a page with maxChars.',
    inputSchema: z.object({ taskId: z.number().int().positive(), offset: z.number().int().nonnegative().default(0), maxChars: z.number().int().min(1).optional() }),
